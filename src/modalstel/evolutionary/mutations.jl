...
function apply_mutation_to_dl_rule(
    ...
    d_rules = rulebase(dl)
    return length(d_rules) == 0 ? dl : begin
...
function removeconjunct(r::Rule, args...; rng::AbstractRNG=default_rng())
    ...
    return length(form_rule) == 1 ? r : begin
    ...
    newfeatcond = begin
        ...
        elseif policy == :operator
            ...
            filtered_featcond = filter(
                mc_thresholds->first(mc_thresholds)
                in [metacondition, dual(metacondition)],
...
function randomatom(
    ...
    return treewalk(
        st, alp, rng;
        rng=rng, criterion=tok->(tok isa Atom && arity(tok)==0),
...
inverseatom(st::SyntaxTree, args...; rng::AbstractRNG=default_rng()) =
    treewalk(
        st;
        rng=rng,
