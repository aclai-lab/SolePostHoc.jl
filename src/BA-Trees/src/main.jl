...
# Example
"""
function batrees(f=nothing; dataset_name="iris", num_trees=10, max_depth=10, dsOutput=true)
    if (isnothing(f))
        if (dsOutput)
            WRAP_batrees(nothing, max_depth, dataset_name=dataset_name, num_trees=num_trees)
            t = BAinSoleTree()
            return t
        end
    else
