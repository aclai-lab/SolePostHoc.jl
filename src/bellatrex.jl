...
function bellatrex(
    ...
    for idx = 1:ndatasetinstances
        ...
        ho = @hyperopt for i in
            ...
            prules =
                projectionvectors(hcat(vrules...), X, Y; maxoutdim = ndims, kwargs...)

            # https://juliastats.org/Clustering.jl/dev/kmeans.html
            # 5) Clustering using k-means++ method
            #println("Clustering phase in...")
            krules, cluster_sizes =
                clusteringvectors(prules; rng = rng, nclusters = nclusters, kwargs...)
            ...
            local_predictions = [
                first(apply(r, currentinstance)) for
                r in rtrees[reduce(vcat, final_rules_idx)]
            ]
            final_prediction = bestguess(
                local_predictions,
                cluster_sizes;
...
function projectionvectors(
    ...
    maxoutdim =
        isnothing(maxoutdim) ? min(size(matrixrules)...) :
