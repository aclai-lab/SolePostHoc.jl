...
function _leaf_extract!(
    ...
    # alphabet = Vector{Vector{LumenAtom{R,T}}}(undef, nclasses)
    alphabet = [LumenAtom{R,T}[] for _ in 1:nclasses]
    tbl = Matrix{T}(undef, nrows, nfeats) # apply input, one chunk at a time
    nat = Vector{Int}(undef, nrows) # atoms produced by row k of the chunk
    preds = Vector{R}(undef, total) # class of every row
    natoms_c = zeros(Int, nclasses)
    ...
    # alphabet = removeduals!.(sort!.(unique!.(atoms)))
    @inbounds for c in 1:nclasses
        _alphabet!(alphabet[c], atoms[c])
    end
...
function _extract!(
    ...
    # _rect_size(lo, hi, config.M) ≤ config.M &&
        ...
        return _leaf_extract!(config, out, thrs, ensemble, lo, hi)
    ...
    _extract!(config, out, thrs, ensemble, lo, hi)
    hi[jstar] = old_hi
...
function _extract(
    ...
    return _extract!(config, out, thrs, ensemble, copy(lo), copy(hi))
end
