...
"""
    _normalize_atom(atom::SL.Atom{<:SD.ScalarCondition}) -> SL.Atom{<:SD.ScalarCondition}
...
"""
function _normalize_atom(atom::SL.Atom{<:SD.ScalarCondition})::SL.Atom{SD.ScalarCondition}
    op = get_operator(atom)

    # fast path: already in the canonical family
    op in ((<), (≥)) && return atom

    thr  = Float64(get_threshold(atom))
    ...
    new_mc = SD.ScalarMetaCondition(
        SD.VariableValue(feat.i_variable, feat.i_variable), new_op
    )
    @show SL.Atom(SD.ScalarCondition(new_mc, new_thr)) isa Atom{<:ScalarCondition}
...
"""
    _thrs_with_boundary(thresholds::Vector{Float64}, family::Symbol) -> Vector{Float64}

Append the appropriate boundary point to `thresholds` depending on the operator family,
ensuring that all `n + 1` ordinal regions induced by `n` thresholds are sampled.

- `:lt` family (`<`/`≤`): thresholds sorted **descending** → appends
  `prevfloat(last(thresholds))` to cover the region **below the smallest threshold**
  (i.e. values smaller than every condition).

- `:gt` family (`>`/`≥`): thresholds sorted **ascending** → appends
  `nextfloat(last(thresholds))` to cover the region **above the largest threshold**
  (i.e. values larger than every condition).
...
Element-wise version: applies `_thrs_with_boundary` to each per-feature threshold
vector using the corresponding operator family.
"""
function _thrs_with_boundary(thresholds::Vector{Float64}, family::Symbol)
    isempty(thresholds) && return [NaN]
...
@inline _thrs_with_boundary(
    thresholds::Vector{Vector{Float64}},
    op_families::Vector{Symbol}
) = _thrs_with_boundary.(thresholds, op_families)
...
The high-level constructor:
1. Extracts atoms from the model (respecting the `depth` parameter).
2. Normalizes all atoms to the canonical `<`/`≥` family via [`_normalize_atom`](@ref).
...
struct ExtractRulesData
    grp_truths::Vector{Vector{Vector{BitVector}}}
    thresholds::Vector{Vector{Float64}}
    features::AbstractVector{<:SM.Label}
    classnames::AbstractVector{<:SM.Label}
    op_families::Vector{Symbol}
    ...
    ExtractRulesData(
        ...
        classnames::AbstractVector{<:SM.Label},
        op_families::Vector{Symbol}
    ) = new(grp_truths, thresholds, features, classnames, op_families)
    ...
    function ExtractRulesData(extractor::LumenConfig, model::SM.AbstractModel)
        ...
        @show typeof(atoms)
        @show atoms isa Vector{<:Atom{<:ScalarCondition}}
        ...
        #             encoding, where larger thresholds correspond to later bits).
        # -------------------------------------------------------------------- #
        ...
        # -------------------------------------------------------------------- #
        return new(grp_truths, thresholds, featurenames, classnames, op_families)
    end
end
...
function get_atoms(
    ...
    Threads.@threads for i in eachindex(truths)
        conjuncts[i] = generate_disjunct(truths[i], thresholds, features, op_families)
    end
