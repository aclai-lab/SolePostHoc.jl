...
"""
    _intrees_prunerule(rc::Rule)::Rule
...
[`rulemetrics`](@ref).
"""
_intrees_prunerule(r::Rule, X, y; kwargs...) =
    _prune_rulez(typeof(antecedent(r)), r, X, y; kwargs...)

function _prune_rulez(::Type{<:Atom}, r::Rule{O}, args...; kwargs...) where {O}
    _intrees_prunerule(
        Rule(LeftmostConjunctiveForm([antecedent(r)]), consequent(r), info(r)),
        args...;
        kwargs...,
    )
end

function _prune_rulez(
    ...
    for idx in reverse(valid_idxs)
        ...
        rule =
            Rule(LeftmostConjunctiveForm(SoleLogics.grandchildren(antd)[other_idxs]), cons)
    ...
    prunedr =
        Rule(LeftmostConjunctiveForm(SoleLogics.grandchildren(antd)[valid_idxs]), cons)
    # @show prunedr
    return prunedr
end

function _prune_rulez(::Type{<:MultiFormula}, r::Rule{O}, args...; kwargs...) where {O}
