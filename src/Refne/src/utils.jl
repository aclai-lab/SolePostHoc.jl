...
function generate_univers_of_combinations(
    min_values::Vector,
    max_values::Vector,
    n_points::Int = 100,
)
...
function generate_univers_of_combinations_ott(
    min_values::Vector,
    max_values::Vector,
    n_points::Int = 100,
)
    ...
    if total_combinations <= reasonable_limit
        ...
        dim_values = [
            collect(range(min_val, max_val, length = n_points)) for
            (min_val, max_val) in zip(min_values, max_values)
        ]

        # Optimized generation
        Threads.@threads for i = 1:total_combinations
            linear_idx = i - 1
            for dim = 1:n_dims
        ...
        center_indices = [max(1, (n_points + 1) ÷ 2) for _ = 1:n_dims]
...
function generate_symbolic_grid_samples(
    min_values::Vector,
    max_values::Vector,
    n_points::Int,
    ...
    for focus_dim = 1:n_dims
        for value_idx = 1:n_points
            if sample_idx > target_samples
                break
            end
